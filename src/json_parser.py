...
class InstagramDataParser:
    ...
    def parse(self, mode: str = 'full', sample_size: int = 50) -> List[Dict[str, Any]]:
        ...
        # Return posts based on mode
        if mode == 'sample':
            selected = all_posts[:sample_size]
            logger.info(f"Sample mode: returning first {len(selected)} posts")
            return selected
        else:
            logger.info(f"Full mode: returning all {len(all_posts)} posts")
            return all_posts
    
    def _is_valid_entry(self, entry: Dict[str, Any]) -> bool:
        """Check if entry has minimum required fields"""
        required = ['id', 'type', 'url', 'ownerUsername']
